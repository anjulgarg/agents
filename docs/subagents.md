...
Pi's native retry policy is enabled by default. After native retries are exhausted, the subagent lifecycle schedules hidden recovery turns that preserve the current session context instead of treating a failed assistant response as successful or immediately killing the child. Recovery continues for up to one minute from the first consecutive transient failure, with exponential backoff capped at 30 seconds. An optional attempt cap remains available for tests and local safety overrides, but the normal budget is time-based. Operators may raise the native budget in their local Pi settings with `retry.maxRetries` and `retry.provider.maxRetries`; personal settings remain outside this repository.

The recovery classifier retries transient HTTP 429 and 5xx responses, upstream connection, socket, timeout, overload, rate-limit, and unstructured Responses API failures such as `Unknown error (no error details in response)`. Authentication, quota, invalid-request, content-policy, and context failures remain fail-fast. Successful assistant calls reset the one-minute window, so intermittent Azure failures do not consume an earlier outage budget. Intermediate transient failure messages stay out of the F6 transcript; if the window expires, the final provider diagnostic is shown to the parent. A new user turn, model change, or new session starts a fresh window.
