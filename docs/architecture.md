...
The `/changes` extension is a read-only, Git-only browser for the union of uncommitted and unpushed files. Its full-screen TUI presents horizontally switchable file tabs, exact colorful diffs with collapsed or full-file context, keyboard scrolling, and an edit-request shortcut that pre-fills Pi's input with the selected path. It performs no model calls.
